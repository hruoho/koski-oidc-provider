...
export class DynamodbAdapterV2 implements Adapter {
    name: string;
    ...
    async upsert(id: string, payload: AdapterPayload, expiresIn?: number): Promise<void> {
        ...
        await dynamoClient.update(params).promise().catch(e => console.log(e));
    }
    ...
    async revokeByGrantId(grantId: string): Promise<void> {
        ...
        do {
            ...
            const batchWriteParams: AWS.DynamoDB.DocumentClient.BatchWriteItemInput = {
                RequestItems: {
                    [TABLE_NAME]: items.reduce<AWS.DynamoDB.DocumentClient.WriteRequests>(
...
export function createDynamoDBAdapter(name: string): AdapterConstructor {
    return class DynamoDBAdapterInstance extends DynamodbAdapterV2 {
        constructor() {
            super(name);
        }
    };
